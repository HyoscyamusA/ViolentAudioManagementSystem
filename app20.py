...
from flask import g
from flask import Flask, render_template, request, redirect, session, jsonify, flash  # 添加 flash 导入
from flask import Flask, render_template, request, redirect, session, jsonify, flash, url_for  # 添加 url_for 导入
from flask import g
...
def get_devices():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('SELECT DISTINCT mac_address AS device_id FROM mac_user')
    devices = [dict(row) for row in cursor.fetchall()]
    conn.close()
