...
import joblib  # 补充缺失的导入
import math  # 补充缺失的导入
from flask import Flask, render_template, request, redirect, session, jsonify, flash, url_for, send_from_directory  # 添加 send_from_directory
...
# 导入自定义模块
from aed import vggish_input, vggish_params, vggish_postprocess, vggish_slim
from utils.console import Console
from utils.common import decode_wav_bytes, read_wav_bytes
...
def get_audio_params(file_path):
    ...
    with sf.SoundFile(file_path) as f:
        ...
        channels = f.channels
        width = f.subtype  # width 可能需要单独定义
...
class AEDetector:
    ...
    def _init_model(self):
        """初始化VGGish模型"""
        try:
            self.console.log_highlight("初始化AED模型...")
            tf.compat.v1.reset_default_graph()
            self.sess = tf.compat.v1.Session()
            
            vggish_slim.define_vggish_slim(training=False)
            ckpt_path = MODEL_PATH / "vggish_model.ckpt"
            vggish_slim.load_vggish_slim_checkpoint(self.sess, str(ckpt_path))
            
            self.pproc = vggish_postprocess.Postprocessor(str(MODEL_PATH / "vggish_pca_params.npz"))
            
            self.gmm_violence = joblib.load(MODEL_PATH / 'zhenglei.model')
            self.gmm_non_violence = joblib.load(MODEL_PATH / 'fulei.model')
            ...
            self.console.log_success("AED模型初始化完成")
            
        except Exception as e:
            self.console.log_error(f"模型初始化失败: {str(e)}")
            raise

    def analyze(self, wav_bytes: bytes, sample_rate: int, channels: int, width: int) -> float:
        """分析音频返回暴力概率"""
        try:
            waveform = decode_wav_bytes(wav_bytes, channels, width)
            waveform = waveform.T / 32768.0

            examples = vggish_input.waveform_to_examples(waveform, sample_rate)
            features = self.sess.run(self.output_tensor, feed_dict={self.input_tensor: examples})
            
            processed_features = self.pproc.postprocess(features)
            
            score_v = self.gmm_violence.score(processed_features)
            score_nv = self.gmm_non_violence.score(processed_features)
            
            max_score = max(score_v, score_nv)
            return math.exp(score_v - max_score) / (math.exp(score_v - max_score) + math.exp(score_nv - max_score))
        
        except Exception as e:
            self.console.log_error(f"音频分析失败: {str(e)}")
            raise
...
def handle_upload():
    ...
    try:
        ...
        # 获取音频参数
        sample_rate, channels = get_audio_params(save_path)
        ...
        # 处理分析（不需要 width）
        violence_prob = aed.analyze(wav_bytes, sample_rate, channels, 2)  # width 设为 2 作为默认值
        
        return jsonify({
            'status': 'success',
            'probability': round(violence_prob, 4),
            'non_violence_probability': round(1 - violence_prob, 4),
...
app.config['UPLOAD_FOLDER'] = './uploads'  # 或 './static/uploads'
# 添加CORS头解决跨域问题
@app.after_request
