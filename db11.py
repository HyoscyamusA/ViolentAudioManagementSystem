...
def create_test_data(db_path):
    ...
    try:
        ...
        c.execute("DELETE FROM user WHERE role != 'admin'")  # 保留管理员
        ...
        for i in range(1, NUM_USERS + 1):
            ...
            # 确保用户ID从2开始（管理员ID为1）
            c.execute("INSERT INTO user (username, password, role) VALUES (?, ?, ?)",
                     (username, "user123", "user"))
            users.append(c.lastrowid)  # 获取自动生成的用户ID
        ...
        conn.commit()
        print(f"成功生成数据：")
        print(f"- 管理员：1 个（固定账户 admin/root）")
        print(f"- 普通用户：{NUM_USERS} 个")
