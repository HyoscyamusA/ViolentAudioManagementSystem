...
def device_manage():
    ...
    if role == 'admin':
        # admin 可以看到所有设备
        cursor.execute('SELECT mac.mac_address, mac_user.device_name, user.username FROM mac '
                       'JOIN mac_user ON mac.mac_address = mac_user.mac_address '
                       'JOIN user ON mac_user.user_id = user.id')
    else:
        # user 只能看到自己绑定的设备
        cursor.execute('SELECT mac.mac_address, mac_user.device_name FROM mac '
                       'JOIN mac_user ON mac.mac_address = mac_user.mac_address '
                       'WHERE mac_user.user_id =?', (user_id,))
