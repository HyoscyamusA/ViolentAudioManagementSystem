...
def init_db():
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS events (
                        ...
                        event_date TEXT,
                        device_name TEXT,
                        PRIMARY KEY (device_id, event_date))''')
...
def insert_event(device_id, event_date, device_name):
    """插入事件记录"""
    conn = sqlite3.connect(DATABASE)
    cursor = conn.cursor()
    try:
        cursor.execute('''INSERT INTO events (device_id, event_date, device_name) 
                          VALUES (?, ?, ?)''', (device_id, event_date, device_name))
        conn.commit()
    except sqlite3.IntegrityError:
        print(f"Error: Event with device_id {device_id} and event_date {event_date} already exists.")
    finally:
        conn.close()
...
def upload_file():
    ...
    try:
        ...
        event_date = upload_time.strftime('%Y-%m-%d')
        
        if violent_probability > 0.5:
            insert_event(device_id, event_date, device_name)
...
def get_line_chart_data():
    try:
        conn = sqlite3.connect(DATABASE)
        cursor = conn.cursor()
        cursor.execute('''SELECT device_id, event_date, COUNT(*) 
...
def get_heatmap_data():
    try:
        ...
        conn = sqlite3.connect(DATABASE)
        cursor = conn.cursor()
        cursor.execute('''SELECT strftime('%m', event_date) AS month, 
