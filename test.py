...
with tf.compat.v1.Session() as sess:  
    ...
    for root, dirs, files in os.walk(main_audio_folder):  
        for sub_dir in dirs:  
            ...
            feature_sub_folder = os.path.join(feature_folder, sub_dir)  # 特征保存文件夹路径  
            
            if not os.path.exists(feature_sub_folder):  
                os.makedirs(feature_sub_folder)  # 创建文件夹  
            ...
            for audio_file in os.listdir(sub_audio_folder):  
                if audio_file.endswith(".ogg"):  # 确保是.ogg文件  
                    ...
                    try:  
                        ...
                        feature_file_path = os.path.join(feature_sub_folder, feature_file_name)  
