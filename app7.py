...
from pathlib import Path
from flask import Flask, render_template, request, redirect, session, jsonify, flash, url_for, send_from_directory, send_file
# 初始化应用
app = Flask(__name__)
...
vggish_slim.load_vggish_slim_checkpoint(sess, "aed/vggish_model.ckpt")
...
def IsViolent(audio_file):
    ...
    # 后处理提取的特征
    pproc = vggish_postprocess.Postprocessor("aed/vggish_pca_params.npz")
    ...
    gmm2 = joblib.load(os.path.join('aed/fulei.model'))  # 暴力事件的GMM模型
    gmm1 = joblib.load(os.path.join('aed/zhenglei.model'))  # 非暴力事件的GMM模型
...
def events_all():
    ...
    base_query = '''
        SELECT 
            ...
            mu.device_name
    '''
    # 修改点1：改用LEFT JOIN获取所有事件
    join_clause = 'FROM events e LEFT JOIN mac_user mu ON e.mac_address = mu.mac_address '
    where_clause = []
    params = []

    if role == 'admin':
        base_query += ', u.username '
        # 修改点2：管理员关联用户表时使用LEFT JOIN
        join_clause += 'LEFT JOIN user u ON mu.user_id = u.id '
    else:
        # 修改点3：使用OR条件包含未绑定设备（根据需求决定是否添加）
        # where_clause.append("(mu.user_id =? OR mu.user_id IS NULL)")
        # params.append(user_id)
        # 原权限逻辑（仅查看自己绑定的设备）
        where_clause.append("mu.user_id =?")
        params.append(user_id)
...
def admin_audio_logs():
    ...
    log_root = "/root/logs"
    selected_mac = request.args.get('mac_address')  # 获取筛选参数
    
    all_macs = set()  # 收集所有唯一 MAC 用于筛选下拉框
    audio_files = []
    ...
    for root, dirs, files in os.walk(log_root):
        for file in files:
            ...
            full_path = str(Path(root) / file)  # 构建完整路径
            # 用 pathlib 生成相对路径
            relative_path = str(Path(full_path).relative_to(Path(log_root)))
            stat = os.stat(full_path)
            
            # 从文件名提取 MAC 部分（假设文件名最后一部分为 MAC，如 `xxx_e4-5f-01-8a-17-86.wav`）
            file_base = file.rsplit('.', 1)[0]  # 去掉 .wav 后缀
            mac_part = file_base.split('_')[-1] if file_base else None
            
            # 简单验证 MAC 格式（6 组十六进制，用 - 分隔）
            if mac_part and len(mac_part.split('-')) == 6:
                all_macs.add(mac_part)
            else:
                mac_part = None
            
            # 应用筛选条件
            if selected_mac and mac_part != selected_mac:
                continue
            ...
            audio_files.append({
                ...
                "mac_address": mac_part  # 添加 MAC 信息
            })
    
    # 整理 MAC 选项（添加“全部”选项）
    mac_options = [''] + sorted(list(all_macs))
        # 按修改时间倒序排序
    audio_files.sort(key=lambda x: x["modified"], reverse=True)
    return render_template('admin_audio_logs.html', 
                           files=audio_files,
...
def admin_serve_audio(path):
    ...
    # 构造完整路径
    full_path = os.path.join("/root/logs", path)
...
if __name__ == '__main__':
    init_db()
    # 添加 threaded=True 启用多线程模式
    app.run(host='0.0.0.0', port=5000, debug=True, threaded=True)  # 修改这行