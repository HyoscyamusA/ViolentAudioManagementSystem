...
def get_heatmap_data():
    try:
        ...
        # 获取事件日期和设备ID (假设是暴力事件)
        cursor.execute('''SELECT event_date, device_id FROM events WHERE is_violent = 1''')  # 仅获取暴力事件
        ...
        # 将数据转换为 DataFrame
        df = pd.DataFrame(data, columns=["dt", "device_id"])

        # 统计每个设备在每个日期的事件次数
        df['count'] = 1  # 为每个事件添加一个计数列
        df = df.groupby(["dt", "device_id"]).sum().reset_index()

        # 为热力图准备数据：我们需要将设备ID转化为一个类似于时间轴的数值
        # 将设备ID转为数字
