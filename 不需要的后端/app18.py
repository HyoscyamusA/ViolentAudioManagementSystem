...
def get_heatmap_data():
    try:
        # 获取设备ID
        device_id = request.args.get('device_id', 'unknown')  # 默认为 "unknown"
        print(f"请求的设备ID: {device_id}")  # 打印设备ID
        ...
        # 查询该设备的所有事件数据
        cursor.execute('''SELECT event_date, device_id, event_count FROM events WHERE device_id = ? AND event_count > 0''', (device_id,))
        data = cursor.fetchall()
        ...
        if not data:
            print(f"没有找到设备ID {device_id} 的数据")
        ...
        # 统计每个设备在每个日期的事件次数
        df['month'] = df['event_date'].dt.month_name().str[:3]  # 获取月份（缩写）
        df['weekday'] = df['event_date'].dt.day_name().str[:3]  # 获取星期（缩写）

        # 重新整理数据，按月份和星期分组，只对事件计数进行求和
        df_grouped = df.groupby(['month', 'weekday'])['event_count'].sum().reset_index()

        # 打印处理后的 DataFrame
        print(f"处理后的 DataFrame: {df_grouped}")
