...
def get_heatmap_data():
    try:
        ...
        if not data:
            ...
            return jsonify({'message': f'没有找到设备ID {device_id} 的数据'}), 404
        ...
        # 提取每月的日期和月份信息
        df['day'] = df['event_date'].dt.day  # 获取日期
        df['month'] = df['event_date'].dt.month_name().str[:3]  # 获取月份（缩写）

        # 汇总数据按日期和月份
        df_grouped = df.groupby(['month', 'day'])['event_count'].sum().reset_index()
