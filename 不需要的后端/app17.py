...
def get_heatmap_data():
    try:
        # 从前端接收设备ID
        device_id = request.args.get('device_id', 'unknown')  # 获取设备号参数，默认为 "unknown"
        print(f"请求的设备ID: {device_id}")  # 打印设备ID以确认请求内容
        ...
        # 确保 `event_date` 列被正确解析为日期格式
        df['event_date'] = pd.to_datetime(df['event_date'], errors='coerce')

        # 检查是否成功创建 DataFrame，并处理日期
        print(f"获取到的DataFrame:\n{df}")
        ...
        # 重新整理数据，方便前端处理
        df_grouped = df.groupby(['month', 'weekday']).sum().reset_index()
