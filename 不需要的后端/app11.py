...
def get_heatmap_data():
    try:
        ...
        conn = sqlite3.connect(DATABASE)
        cursor = conn.cursor()
        cursor.execute("SELECT event_date, device_id, event_count FROM events ORDER BY event_date")  # 获取日期和设备ID及事件计数
        events = cursor.fetchall()
        conn.close()

        # 格式化数据，使其符合前端需要的格式
        formatted_events = [{'date': event[0], 'device_id': event[1], 'count': event[2]} for event in events]
