...
class Aed:
    def __init__(self) -> None:
        ...
        # 修正检查点路径
        checkpoint_path = os.path.join(MODEL_PATH, "vggish_model.ckpt")
...
@app.route('/audio/upload', methods=['GET'])
def audio_upload():
...
def upload_audio():
    try:
        ...
        if audio_file:
            # 读取音频文件
            str_data = audio_file.read()
            # 假设采样率、通道数和宽度可以从文件中正确获取（需根据实际情况调整）
            framerate = 44100  # 示例采样率
            num_channel = 1    # 示例通道数
            num_sample_width = 2  # 示例宽度

            # 计算暴力事件概率（调用Aed模型）
            violence_probability = aed.decode(str_data, framerate, num_channel, num_sample_width)
            non_violence_probability = 1 - violence_probability
            ...
            return jsonify({
                ...
                'non_violence_probability': non_violence_probability,
                'audio_url': f'/audio/uploads/{filename}'  # 注意路径格式
            })
        else:
            return jsonify({
                'status_code': 40000,
